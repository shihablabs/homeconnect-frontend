...
export function EscrowManagementClient() {
  ...
  const handleResolve = async (paymentId: string, resolution: 'release' | 'refund') => {
    const isConfirmed = await confirmDelete(
      ...
      `Are you sure you want to ${resolution} this payment?`,
      resolution === 'release' ? 'Release' : 'Refund'
    );
    ...
    try {
      ...
      showSuccess(`Dispute resolved: ${resolution === 'release' ? 'Funds released' : 'Refunded'}`);
      fetchData();
    } catch (error: unknown) {
      const errorMessage = error && typeof error === 'object' && 'response' in error
        ? (error as { response?: { data?: { message?: string } } }).response?.data?.message
        : undefined;
      showError(errorMessage || 'Failed to resolve dispute');
    }
  };
